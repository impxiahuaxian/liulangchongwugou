...
public class UserDao {
	...
	public int addUser(User user, Connection conn){
		String sql = "INSERT INTO user(user_id,user_name,user_pass,real_name,nick_name,user_sex,user_age,user_address,user_phone,user_mail,user_score,user_level,reg_date,user_type) values(null,?,?,?,?,?,?,?,?,?,?,?,?,?)";
	...
	public int updateUser(User user, Connection conn){
		...
		if (!StringUtil.isEmptyString(user.getUser_address())) {
			sBuilder.append(" ,user_address ='" + user.getUser_address() +"' ");
		}
	...
	public int updateUserLevel(User user, Connection conn){
		StringBuilder sBuilder = new StringBuilder();
		sBuilder.append("UPDATE user SET user_id = " + user.getUser_id() +" ");
		if (user.getUser_level()!=0) {
			sBuilder.append(" ,user_level = user_level+" + user.getUser_level() +" ");
		}
